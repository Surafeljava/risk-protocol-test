...
export default async function CardItem({address}) {
    ...
    const response = await fetch(`https://api.0x.org/swap/v1/quote?buyToken=USDC&sellToken=${address.symbol}&sellAmount=${amt}`);
    const data = await response.json();
    return (
        <div className="flex flex-col w-1/2 md:w-1/4 p-2 ">
            <Link href={`/market/${address.name}`} className="flex flex-col rounded-3xl px-6 py-8 h-[300px] hover:scale-[99%] duration-300 group hover:cursor-pointer" 
            style={{backgroundColor:address.color}}>
                ...
                <div className="flex flex-row w-full justify-center items-center flex-grow">
                    <Image src={address.imageUrl} width={180} height={180} className='group-hover:scale-110 duration-300'/>
                </div>
            </Link>
        </div>
    );
